...
static int _GuiRadioButton_getPosition (GuiRadioButton me) {
	...
	static LRESULT CALLBACK _ModernRadioButtonSubclassProc (
		...
		switch (uMsg) {
			...
			case WM_PAINT: {
				...
				// Radio geometry: 18x18px circle, vertically centered
				int boxSize = 18;
				...
				_GuiWin_ensureGdiplus ();
				{
					...
					if (! isEnabled) {
						...
						g.FillEllipse (& disabledBg, x, y, size, size);
						g.DrawEllipse (& disabledBorder, x + 0.5f, y + 0.5f, size - 1.0f, size - 1.0f);
					} else if (isChecked) {
						Gdiplus::SolidBrush whiteBrush (Gdiplus::Color (255, 255, 255, 255));
						g.FillEllipse (& whiteBrush, x, y, size, size);

						Gdiplus::Color borderCol = isPressed ? Gdiplus::Color (255, 0, 90, 168)
							: (isHover ? Gdiplus::Color (255, 0, 120, 215) : Gdiplus::Color (255, 0, 103, 192));
						Gdiplus::Pen activePen (borderCol, 1.5f);
						g.DrawEllipse (& activePen, x + 0.75f, y + 0.75f, size - 1.5f, size - 1.5f);

						// Centered smooth anti-aliased solid dot (8.0f diameter)
						Gdiplus::SolidBrush dotBrush (borderCol);
						g.FillEllipse (& dotBrush, x + 5.0f, y + 5.0f, 8.0f, 8.0f);
					} else {
						...
						g.DrawEllipse (& borderPen, x + 0.6f, y + 0.6f, size - 1.2f, size - 1.2f);
					}
				}
			...
			case WM_NCDESTROY: {
				RemovePropW (hwnd, L"PraatHover");
				RemoveWindowSubclass (hwnd, _ModernRadioButtonSubclassProc, uIdSubclass);
				break;
			}
			default: break;
		}
	...
	conststring32 buttonText, GuiRadioButtonCallback valueChangedCallback, Thing valueChangedBoss, uint32 flags)
{
	...
	my d_next = nullptr;
		...
		SetWindowSubclass (my d_widget -> window, _ModernRadioButtonSubclassProc, 1, 0);
		my v_positionInForm (my d_widget, left, right, top, bottom, parent);
		if (flags & GuiRadioButton_SET) {
